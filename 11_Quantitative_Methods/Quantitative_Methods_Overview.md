...
Entry point for the Quant topic area. **Four readings:** multiple regression, time series, machine
learning, big data projects. **Full reading-level coverage.**
...
- [[Time_Series_Analysis]] — trend (linear/log-linear), covariance stationary, AR, unit root,
  Dickey-Fuller, first differencing, chain rule, RMSE, seasonality, ARCH, cointegration (DF-EG)

## Reading 3 — Machine Learning
- [[Machine_Learning]] — supervised/unsupervised/deep learning; overfitting & bias-variance; LASSO,
  SVM, KNN, CART, random forest; PCA, k-means/hierarchical clustering; neural nets, DLN, RL

## Reading 4 — Big Data Projects
- [[Big_Data_Projects]] — data wrangling & scaling; text processing (BOW, DTM, N-grams); model
  evaluation (confusion matrix, precision/recall/F1, ROC/AUC); bias-variance tuning, grid search
