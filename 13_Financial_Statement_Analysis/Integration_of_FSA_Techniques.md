...
### 2026-07-19 — Why use ROCE to compare profitability across different tax structures?
**Q:** When comparing the profitability of companies in jurisdictions with different tax structures, why should return on capital employed (ROCE) be used?
**A:** `ROCE = operating profit (EBIT) / capital employed`, where capital employed = total assets − non-interest-bearing current liabilities (≈ interest-bearing debt + equity). Because the numerator is a **pre-tax, pre-interest** measure, ROCE is unaffected by differences in statutory tax rates, tax incentives, and deferred-tax positions — so cross-country differences in ROCE reflect **operating performance**, not the tax regime. ROE and ROA use **net income (after-tax)**, so a firm in a low-tax jurisdiction looks artificially more profitable even with identical operations. Being pre-interest also makes ROCE **capital-structure-neutral**, which matters because tax regimes themselves distort financing choices (debt tax shields). Trap: keep numerator/denominator consistent — EBIT pairs with **total capital** (debt + equity), while net income pairs with **equity** (ROE).
